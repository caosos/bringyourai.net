...
def readme_rows() -> list[list[object]]:
    return [
        ...
        ["Source PDF set", "Arvest statements Dec 2024-Nov 2025 and Navy Federal statements Feb 2025-Oct 2025."],
...
def write_outputs(master_rows: list[dict[str, object]], source_rows: list[dict[str, object]], summary_rows: list[dict[str, object]]) -> None:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Keith court evidence ledger from bank statement PDFs.")
    ...
    write_outputs(master_rows, source_rows, summary_rows)
    ...
    print(f"CSVs: {MASTER_CSV}, {SUMMARY_CSV}, {SOURCE_INDEX_CSV}")
    if not source_rows:
        print(f"No PDFs found in {args.pdf_dir}; place Keith's statement PDFs there and rerun.")
    return 0
